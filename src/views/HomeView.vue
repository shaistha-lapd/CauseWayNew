...
export default {
  ...
  data: () => ({
    ...
    incomplete: {
      ...
      smallIconColor: "red",
      caption:
        "Unfinished FIs on this device only. There may be others stored on other computers you have used.",
    ...
    rejected: {
      ...
      caption:
        "These FIs were rejected by the server. Please review for any errors and resubmit",
      route: "/fi/activity/incompletes",
      data: "",
    },
    saved: {
      ...
      smallIconColor: "primary",
      caption:
        "Completed FIs that have been saved to the local device and are ready to submit",
  ...
  watch: {
    appUser(v) {
      if (v.serial) {
        this.$store
          .dispatch("Fi/getIncompleteFiFromDb", v)
          .then((res) => (this.incomplete.data = res));
        this.$store
          .dispatch("Fi/getRejectedFiFromDb", v)
          .then((res) => (this.rejected.data = res));
        this.$store
          .dispatch("Fi/getDashboardStats", this.appUser.serial)
          .then((res) => (this.totals.data = res));
      }
    },
  },
  ...
  created() {
    if (this.appUser.serial) {
      this.$store
        .dispatch("Fi/getIncompleteFiFromDb", this.appUser)
        .then((res) => (this.incomplete.data = res));
      this.$store
        .dispatch("Fi/getRejectedFiFromDb", this.appUser)
        .then((res) => (this.rejected.data = res));
      this.$store
        .dispatch("Fi/getDashboardStats", this.appUser.serial)
        .then((res) => (this.totals.data = res));
