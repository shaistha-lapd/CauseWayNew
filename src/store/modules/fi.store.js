...
export default {
  ...
  mutations: {
    CREATE_FI(state, user) {
      ...
      state.fi.incident.incNumber = `${tempDate[0].substring(2)}${tempDate[1]}${
        tempDate[2]
      }00`;
    ...
    ADD_OFFICER(state, ofcr) {
      if (
        !state.fi.incident.incOfficers.find((o) => o.serial == ofcr.serial) &&
        ofcr.serial != null
      ) {
    ...
    UPDATE_NARRATIVE(state) {
      if (
        !!state.fi.person.homelessRationale &&
        !state.fi.incident.incNarrative.includes("HR:")
      ) {
        state.fi.incident.incNarrative +=
          "\nHR: " + state.fi.person.homelessRationale + "\n";
      }
      if (
        !!state.fi.person.caJustification &&
        !state.fi.incident.incNarrative.includes("CA:")
      ) {
        state.fi.incident.incNarrative +=
          "CA: " + state.fi.person.caJustification + "\n";
      }
    },
  },
  actions: {
    ...
    async getFiFromDb(id) {
      ...
      if (!fi) {
          ...
          `https://lapd-cdnjs.azurewebsites.us/api/getById?id=${id}`
        )
    ...
    async getUserFis(user) {
      let list = await fetch(
        `https://lapd-cdnjs.azurewebsites.us/api/getFisByOfficerId?serialNum=${user.serial}`
      )
        .then((response) => response.json())
        .then((data) => {
          return data;
    ...
    async getIncompleteFiFromDb(user) {
      let incs = await db.FIs.where("[fi.data.status+fi.data.creator.serial]")
        .equals(["ABANDONED", user.serial])
        .reverse()
        .toArray();
      return incs;
    },
    ...
    async submit({ commit, dispatch }) {
      if (navigator.onLine) {
        ...
        let fiList = await db.FIs.where("fi.data.status")
          .equals("READY")
          .toArray();
        ...
        let msg =
          "Currently there is no network signal. Your FI's will be stored and sent when your network returns.";
    ...
    deleteFromDex(id) {
      db.FIs.where("id")
        ...
        .catch((e) =>
          console.log(
            `There was a problem deleting the record with ID: ${id} \n ${e}`
          )
        );
    },
    ...
    purgeFromDex() {
      db.FIs.where("fi.data.status")
        ...
        .catch((e) => {
          console.error(
            e.failures.length +
              " items were not accepted by the server. Please try again."
          );
        });
    },
    async postFi(tempFi) {
      const res = await fetch(
        "https://lapd-cdnjs.azurewebsites.us/api/postFi",
        {
          method: "POST",
          headers: {
      ...
      Notification.requestPermission().then((permission) => {
        ...
        navigator.serviceWorker.ready.then((registration) => {
          registration.showNotification("Success!", {
            body:
              "Browser Notification - " + count + " FI(s) sent successfully",
          });
        });
      });
    },
    async getDashboardStats(serial) {
      ...
      let stats = await fetch(
        `https://lapd-cdnjs.azurewebsites.us/api/getDashboardStats?code=${code}&serial=${serial}`
      )
        .then((res) => res.json())
        .then((data) => {
          return data;
    ...
    async getEmpsByDiv(div) {
      const authCode =
        "jMRG-BPQh7oCmxzR9tioRdbtjen4jDnbHzDclreitrVaAzFu85P4wA==";
      return fetch(
        `https://lapd-cdnjs.azurewebsites.us/api/getEmpByDiv?code=${authCode}&div=${div}`
      )
        .then((res) => res.json())
        .then((data) => {
          return data;
